...
export function AppShell() {
  ...
  const demoMode = session?.id.startsWith('FAM-7X');
  ...
  return <div className={`app-shell ${presentationMode?'is-presentation':''}`}>
    ...
    <main className="app-main">
      <div className="topline"><div><span className="tracking-pill"><i/> TRACKING ACTIVE</span>{demoMode&&<span className="demo-mode-indicator" role="status">SIMULATED DEMO</span>}<span>{session.name}</span><span className="top-event">{session.eventName}</span></div><div><span>{members.length} members</span><span role="status" aria-label={`Network status: ${online?'online':'offline'}`}>{online?<Wifi/>:<WifiOff/>}{online?'Online':'Offline'}</span><span><Bluetooth/>Optional BLE</span></div></div>

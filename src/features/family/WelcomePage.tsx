...
function NetworkArtwork() { const nodes = [{x:52,y:17,label:'Guardian',icon:<Smartphone/>},{x:20,y:49,label:'Band 01',icon:<Radio/>},{x:79,y:46,label:'Meera',icon:<Smartphone/>},{x:44,y:79,label:'Band 02',icon:<Bluetooth/>}]; return <div className="network-art" aria-label="Illustration of connected family devices"><div className="network-orbit"/><div className="network-safe-ring"/><svg viewBox="0 0 100 100" aria-hidden="true"><path d="M52 17 L20 49 L44 79 L79 46 Z"/><path d="M52 17 L44 79 M20 49 L79 46"/></svg>{nodes.map((node,i)=><motion.div key={node.label} className={`network-node node-${i}`} style={{left:`${node.x}%`,top:`${node.y}%`}} animate={{y:[0,-7,0]}} transition={{duration:3+i*.4,repeat:Infinity}}>{node.icon}<span>{node.label}</span>{i===3&&<i/>}</motion.div>)}<div className="map-pulse"><MapPin/><span>Reunion point</span></div><div className="network-label"><Users/><span>4 people</span><strong>All close</strong></div></div>; }
